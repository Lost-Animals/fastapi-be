...
from db import db
from schemas.image_schemas import ImageCreate, AnimalImage


images_ref = db.collection("images")


async def insert_image(data: ImageCreate) -> AnimalImage:
    image_data_dict = data.model_dump()
    new_image_ref = images_ref.document()
    await new_image_ref.set(image_data_dict)

    return AnimalImage(id=new_image_ref.id, **image_data_dict)


async def select_images_by_animal_id(animal_id: str) -> list[AnimalImage]:
    ...
    return [AnimalImage(id=doc.id, **doc.to_dict()) for doc in docs]
